...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        // 检查是否是目标节点
        if (![NODE_NAMES.IMAGE_LOADER, NODE_NAMES.VIDEO_LOADER, NODE_NAMES.IMAGE_SAVER, NODE_NAMES.IMAGE_PREVIEW].includes(nodeData.name)) {
            return;
        }
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            const getDirectoryWidget = () => {
                return (this.widgets || []).find((w) => w.name === "目录路径" || w.name === "输出目录" || w.name === "保存路径");
            };
